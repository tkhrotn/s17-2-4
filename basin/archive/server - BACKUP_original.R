server <- function(input, output, clientData, session){
  ...
  # srclatCT <- reactive({srclatCT <- input$map_click$lat; return(srclatCT); })
  # srclngCT <- reactive({srclngCT <- input$map_click$lng; return(srclngCT); })
  # srcCT <- reactive({srcCT <- input$srcCT; return(srcCT); })
  ...
  # output$latlngOutputCT = renderText({
  #   srclatCT <- srclatCT()
  #   srclngCT <- srclngCT()
  #   paste0(srclatCT, "     ", srclngCT)
  # })
  ...
  # output$latCT <- renderInfoBox({
  #   srclatCT <- srclatCT()
  #   infoBox(
  #     "緯度", srclatCT, icon = icon("map", lib = "font-awesome"),
  #     color = "light-blue", fill =TRUE, width = 3)
  # }) 
  # 
  # output$lngCT <- renderInfoBox({
  #   srclngCT <- srclngCT()
  #   infoBox(
  #     "緯度", srclngCT, icon = icon("map", lib = "font-awesome"),
  #     color = "light-blue", fill = TRUE, width = 3)
  # })
  ...
  #   gamm <- skewness()
  #   
  #   getSolc <- function(sec2day=20, dist, maxt, mass, area, disp, velo, gamm) {
  # 
  ...
  #     return(solc)
  #   }
  #   # distance from map_click to station points
  #   d <- map %>% 
  #     as.data.frame %>% 
  #     rename(x=POINT_X, y=POINT_Y) %>% 
  #     extract(c("x", "y")) %>% 
  #     geodist(data.frame(x=srclng,y=srclat),measure="geodesic")
  #   
  #   getSolc_d <- getSolc(sec2day=20, dist=d[1,], maxt=86400, mass=mass, area=area, disp=disp, velo=velo, gamm=gamm)
  #   
  #   
  #   if (is.null(map()) | is.null(data())) {
  #     return(NULL)
  ...
  observe({
    ...
    # if (is.null(srclat()) | is.null(srclng()) | is.null(srcROI()) | is.null(srclatCT()) | is.null(srclngCT()) | is.null(srcCT()) | is.null(map())) {
    # if (is.null(srclatCT()) | is.null(srclngCT()) | is.null(srcCT()) | is.null(map())) {
      return(NULL)
    }
    ...
    isolate({
      ...
      # removeControl(layerId = "srclatlngCT") %>%
      # removeControl(layerId = "srcCT") %>%
      addCircles(
        lng = srclng,
        lat = srclat,
